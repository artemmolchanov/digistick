 ...
 * @since 2.0
 */
class PositionColumn extends Column
{
     ...
     * @see buttons
     */
    public $template = '{move-up} {move-down} {move-to}';
    /**
    ...
    {
        $this->initDefaultButton('move-up', 'arrow-up');
        $this->initDefaultButton('move-down', 'arrow-down');
        $this->initPositionPopupButton('move-to', 'edit');
    }
    ...
    public function initPositionPopupButton($name, $iconName) {
        if (!isset($this->buttons[$name]) && strpos($this->template, '{' . $name . '}') !== false) {
            $this->buttons[$name] = function ($url, $model, $key) use ($iconName) {
                return Html::a(Html::tag(
                    'span',
                    '',
                    [
                        'class' => "edit-position glyphicon glyphicon-$iconName",
                        'sticker-pack-id' => $model->id,
                    ])
                );
            };
        }
